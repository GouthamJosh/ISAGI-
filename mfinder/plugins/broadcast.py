...
try:
    from mfinder.db.db_support import USERS_COLLECTION
except ImportError:
...
async def get_subscribers_count(bot: Client, message: Message):
    ...
    active, blocked = await users_info(bot, USERS_COLLECTION)
    ...
    if "broadcast" in message.text and message.reply_to_message is not None:
        ...
        for row in query:
            ...
            try:
                ...
                LOGGER.error("Broadcast failed for user %s: %s", chat_id, e)
            if br_msg: 
                success += 1
            else:
                failed += 1
