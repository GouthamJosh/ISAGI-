...
async def admin_settings_(bot, update):
    ...
    auto_delete = admin_settings.get("auto_delete", 0)
    custom_caption = admin_settings.get("custom_caption")
    fsub_channel = admin_settings.get("fsub_channel")
    caption_uname = admin_settings.get("caption_uname")
    invite_link = admin_settings.get("channel_link")
    repair_mode = admin_settings.get("repair_mode", False)
    admins = ""
    dbchannel = ""
    for admin in ADMINS:
        admins += "\n" + "`" + str(admin) + "`"
